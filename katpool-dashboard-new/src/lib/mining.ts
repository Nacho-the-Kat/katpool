...
export function miningConfig(): MiningConfig {
  const network: PoolNetwork = env("NEXT_PUBLIC_POOL_NETWORK") === "testnet-10" ? "testnet-10" : "mainnet";
  const host = env("NEXT_PUBLIC_STRATUM_HOST") ?? DEFAULT_HOST;
  const domain = baseDomain(host);

  const ports = env("NEXT_PUBLIC_STRATUM_PORTS")
    ? parsePorts(env("NEXT_PUBLIC_STRATUM_PORTS") as string)
    : DEFAULT_PORTS;
  ...
  const showRegions = host === DEFAULT_HOST && env("NEXT_PUBLIC_STRATUM_PORTS") === undefined;
