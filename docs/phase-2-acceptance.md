...
| 4 | Importer maps every production legacy table into the new schema. | `katpool-import-legacy` binary with 5 transforms: blocks, balances, payments, nacho_payments, krc20. | GREEN — landed in PR #13 (part A) + PR #14 (part B) |
...
| 7 | Importer scales linearly enough to finish a production cutover inside the 30-minute window. | `cargo test -p katpool-import-legacy --test import_scale` (CI-default 1K block run; `--ignored` 10K block run). Empirical: 2.4 ms/block on the laptop CI runner. Extrapolates to ~21.6 min for the production 539K blocks. | GREEN — measured 2026-05-26 |
| 8 | Cross-cutting invariants survive partial-failure restarts and idempotent re-runs against a mutating legacy DB. | `cargo test -p katpool-import-legacy --test import_properties` (5 tests). | GREEN — landed in this PR (milestone 4) |
| 9 | Operator rehearsal script captures the cutover-evidence artefacts required by the cutover ticket. | `scripts/legacy-importer-rehearsal.sh` writes reconcile JSON + tracing log + audit-log snapshot + signed manifest (git rev + binary sha256) into a timestamped artefact directory. | GREEN — landed in this PR |
| 10 | Cutover plan ([`docs/cutover-plan.md`](cutover-plan.md)) references the new importer + rehearsal script, not the placeholder path it inherited from the original plan. | T-2m step in the cutover timeline points at `scripts/legacy-importer-rehearsal.sh --no-dry-run` with the `manifest.reconcile_all_passed == "true"` gate. | GREEN — landed in this PR |
| 11 | Operator runbook documents dry-run, hot-run, and partial-failure-restart flows. | [`docs/runbooks/14-legacy-importer.md`](runbooks/14-legacy-importer.md). | GREEN — landed in PR #14, expanded in this PR with the rehearsal script |
