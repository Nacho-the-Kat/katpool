...
| 9 | Operator rehearsal: one full dry-run cycle produces reconcile JSON + audit log + manifest (mirrors Phase 2 importer pattern). | `katpool-payout-rehearsal` one-shot tool (`PayoutEngine` dry-run) + `scripts/kas-payout-rehearsal.sh` + [runbook 18](runbooks/18-kas-payout-rehearsal.md). Reconcile-envelope builder unit-tested for the exact JSON contract; live testnet-10 dry-run executed by the operator per the runbook, artefacts archived under `payout-evidence/`. | GREEN (tool) — M4.8; live run operator-driven |
