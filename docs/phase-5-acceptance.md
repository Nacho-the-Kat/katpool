...
| 4 | Sign + submit commit/reveal via kaspad gRPC; drive `krc20_pending_transfer` (`pending → commit_submitted → reveal_submitted → completed` / `failed`); record intent **before** broadcast; no double-pay on mid-cycle restart. | Deterministic txscript-engine verification + mock-kaspad orchestration on testcontainer Postgres; crash-before-broadcast chaos test. | IN PROGRESS — M5.4a (signer: commit standard, reveal manual P2SH-redeem path, both engine-verified, deterministic txids) GREEN; M5.4b (executor state machine + orchestration + chaos) PENDING |
